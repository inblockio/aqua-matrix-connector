...
- [x] PHYSICAL SPLIT into ~/aqua-agents. Connector ac66f9f (5 crates, build 0 warn, test 31/0, dep-check OK). aqua-agents 6e3ef80 (3 crates template/heartbeat/claude-p, build 0 warn, test 16/0). Cross-repo path-deps: aqua-agents → ../aqua-matrix-agent/crates/{relay,orchestrator,gating} + template self (siwx-oidc-auth pattern). types/instances/Dockerfile/build-image.sh moved. Skills/+systemd/ stayed in connector (deployment-migration follow-up). No secrets tracked. Binaries NOT renamed. 47 tests preserved (31+16).
- [x] COMMIT + PUSH both repos. aqua-agents main 020e5a2..6e3ef80 pushed; connector branch refactor/connector-agents-split pushed (PR: github.com/inblockio/aqua-matrix-agent/pull/new/refactor/connector-agents-split).
- [x] E2E with my own checker agent — PASS. Throwaway identities (live HS, authorized): checker @did-key-z6mkfpx…dfhbka, heartbeat @did-key-z6mktevr…cltuj, claude-p @did-key-z6mkf6xp…53w3f. Round-trip: `#shell ping`→`pong @ 2026-06-02 15:55:40Z`; `#shell status`→host-status block; cmds counter incremented; OpsHandler logged sender==target (authorize ✓). Hello DM received + Megolm-decrypted in a SINGLE DM room (!cETQcKNjntWTdsAqDl) — no dup-room regression. claude-p connects+registers+run-loop (full reply degraded only by absent CLAUDE_CODE_OAUTH_TOKEN). Container spawn validated-to-gate (orchestrator reachable, 2 types loaded, cmd_spawn wired) but NOT executed — type defs need a token whose EnvOverrideThenTokenFile fallback would consume the USER's prod token + 2 live prod containers already running (correct skip). §2 invariants: NO regressions. All test daemons killed; live services untouched; artifacts in /tmp only. Minor: one test heartbeat needed SIGKILL after SIGTERM during cleanup (worth verifying the SIGTERM trap on this branch vs memory relay-graceful-shutdown-sigterm).
- [x] Final report to user — see chat.
  >>> MISSION COMPLETE <<<
