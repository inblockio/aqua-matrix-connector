...
| WSL reboots — **podman container fleet** (`aqua-agent-*` consultants + tim-channel) | `aqua-agent-fleet-restore.service` (oneshot, at boot) runs `~/restore-agent-fleet.sh`, which `podman start`s every exited `aqua-agent-*` container. Needed because podman restart policies don't survive a VM reboot and the fleet's `--restart on-failure` never fires on a clean shutdown SIGTERM (exit 0). DIDs/crypto stores live in the persist volumes, so agents come back `store_wiped=false`. | seconds after boot |
| An `aqua-agent-*` container stays down anyway | `crashloop-watch.sh` DMs Tim a CRITICAL "agent DOWN" when a container leaves `running`, then **re-alerts every hour while it stays down** ("agent STILL down", with downtime duration). Alerts are delivery-confirmed: a DM that fails (e.g. network still coming up at boot) is retried every 15s poll until one lands. | first DM ≤15s after the down transition |
